...
export interface QrPaymentModalProps {
  ...
  amount?: number;
  currency?: string;
  /** Si ya existe un cobro pendiente/en revisión, se reanuda en vez de pedir confirmar uno nuevo. */
  resumePaymentId?: string | null;
  confirmTitle: string;
  confirmText: string;
  reviewText: string;
  successText: string;
  /** Crea el cobro en el backend (POST correspondiente) y devuelve el payment resultante. */
  requestCharge: () => Promise<{ payment: any }>;
  onClose: () => void;
  onPaid: () => void;
}

/**
 * Modal genérico de cobro puntual por QR bancario: genera el cobro, muestra
 * el QR (real si el admin lo configuró, o el placeholder de ejemplo si no),
 * deja subir el comprobante ahí mismo, y va consultando el estado hasta que
 * un admin lo confirma o lo rechaza. Reutilizado por propiedad-extra y
 * suscripciones — la diferencia entre ambas es solo qué endpoint crea el
 * cobro y los textos mostrados.
 */
export default function QrPaymentModal({
  visible, title, subject, amount, currency, resumePaymentId,
  confirmTitle, confirmText, reviewText, successText, requestCharge, onClose, onPaid,
}: QrPaymentModalProps) {
  ...
  const createCharge = useCallback(async () => {
    ...
    setErrorMsg('');
    try {
      const { payment } = await requestCharge();
      setPayment(payment);
      setPhase(phaseForStatus(payment.status));
    } catch (e: any) {
        ...
        <View style={styles.sheet}>
          ...
          {phase === 'confirm' && (
            <View style={styles.center}>
              ...
              <Text style={styles.statusTitle}>{confirmTitle}</Text>
          ...
          {phase === 'in_review' && (
            <View style={styles.center}>
              ...
              <Text style={styles.statusText}>{reviewText}</Text>
            </View>
          )}
          ...
          {phase === 'confirmed' && (
            <View style={styles.center}>
              ...
              <Text style={styles.statusText}>{successText}</Text>
              <TouchableOpacity style={styles.retryBtn} onPress={onClose}>
...
const styles = StyleSheet.create({
  ...
  subject: { fontSize: Fonts.sizes.sm, color: Colors.gray[500], marginBottom: 2 },
