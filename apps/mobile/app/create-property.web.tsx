...
// Handles map clicks to drop / move the pin
...
function MapFly({ coord }: { coord: [number, number] | null }) {
  const map = useMap();
  useEffect(() => {
    if (coord) map.flyTo(coord, Math.max(map.getZoom(), 16), { duration: 0.5 });
  }, [coord]);
  return null;
}

// Invalidate size after modal opens so tiles render correctly
function MapResizer({ trigger }: { trigger: boolean }) {
  const map = useMap();
  useEffect(() => {
    if (trigger) setTimeout(() => map.invalidateSize(), 100);
  }, [trigger]);
  return null;
}
...
interface Amenity { id: string; name: string; slug: string; icon: string; category: string; }
...
export default function CreatePropertyWeb() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (selectedAmenities.size > 0) body.amenity_ids = Array.from(selectedAmenities);
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    paddingTop: 24, paddingHorizontal: Spacing.lg, paddingBottom: Spacing.md,
  ...
  mapOverlay: {
    ...
    backgroundColor: Colors.white, zIndex: 9999,
    flexDirection: 'column',
  },
  mapHeader: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between',
    paddingTop: 16, paddingHorizontal: Spacing.lg, paddingBottom: Spacing.md,
    borderBottomWidth: 1, borderBottomColor: Colors.gray[100],
  },
  mapSearchBar: {
  ...
  searchResults: {
    position: 'absolute' as any, top: 130, left: Spacing.lg, right: Spacing.lg,
    backgroundColor: Colors.white, borderRadius: Radius.md,
    boxShadow: '0 4px 16px rgba(0,0,0,0.15)' as any, zIndex: 10000,
  },
  ...
  amenityCatLabel: { fontSize: Fonts.sizes.xs, fontWeight: '700', color: Colors.gray[500], textTransform: 'uppercase' as any, letterSpacing: 1, marginBottom: Spacing.sm },
  amenityGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: Spacing.sm },
  amenityChip: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingVertical: 8, paddingHorizontal: 12, borderRadius: Radius.full, borderWidth: 1.5, borderColor: Colors.gray[200], backgroundColor: Colors.white, cursor: 'pointer' as any },
